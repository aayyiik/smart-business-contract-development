...
@section('dashboard')
                ...
                <div class="mb-3">
                    ...
                    @if ($contract->oe > 100000000)
                        <a href="#" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#svp">Kirim Ke
                            SVP</a>
                    @else
                        <a href="#" class="btn btn-success btn-xs" data-toggle="modal" data-target="#acc">Final
                            Persetujuan</a>
                    @endif
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="d-inline"
                        action="{{ route('vp.contract-return', ['contract' => $contracts->pivot->contract_id, 'vendor' => $contracts->pivot->vendor_id]) }}"
                        method="POST">
                        @csrf
                        @method('post')
    ...
    <div class="modal fade" id="svp" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Kirim ke SVP</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form class="d-inline"
                        action="{{ route('vp.contract-approval', ['contract' => $contracts->pivot->contract_id, 'vendor' => $contracts->pivot->vendor_id]) }}"
                        method="POST">
                        @csrf
                        @method('post')
                        <div class="form-group">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-xs" data-dismiss="modal">Close</button>
                    <button class="btn btn-warning btn-xs" type="submit">Kirim ke SVP</button>
                </div>
                </form>
            </div>
        </div>
